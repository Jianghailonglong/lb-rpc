...
// 系统初始化注册modid/cmdid使用(首次拉取)(初始化使用，只调用一次即可)
int APIClient::reg_init(int modid, int cmdid)
{
    route_set route;
    ...
    if (route.empty() == true) {
        return lbrss::RET_NOEXIST;// 3
    }

    return lbrss::RET_SUCC; // 0
}

APIClient::APIClient():seqid_(0)
{
    printf("APIClient()\n");
    // 1 初始化服务器地址
...
// lbrss 系统获取host信息 得到可用host的ip和port
int APIClient::get_host(int modid, int cmdid, std::string &ip, int &port)
{
    ...
    head.msgid = lbrss::ID_GetHostRequest;
    ...
    int ret = sendto(sockfd_[index], write_buf, head.msglen + MESSAGE_HEAD_LEN, 0, NULL, 0);
    
    if (ret == -1) {
        perror("sendto");
        return lbrss::RET_SYSTEM_ERROR;
    }
    ...
    do {
        message_len = recvfrom(sockfd_[index], read_buf, sizeof(read_buf), 0, NULL, NULL);
        if (message_len == -1) {
            perror("recvfrom");
            return lbrss::RET_SYSTEM_ERROR;
        }
        ...
        if (head.msgid != lbrss::ID_GetHostResponse) {
...
int APIClient::get_route(int modid, int cmdid, route_set &route)
{
    // 1. 封装请求消息
    ...
    MsgHead head;
    head.msglen = req.ByteSizeLong();
    head.msgid = lbrss::ID_API_GetRouteRequest;
    memcpy(write_buf, &head, MESSAGE_HEAD_LEN);
    ...
    int message_len = recvfrom(sockfd_[index], read_buf, sizeof(read_buf), 0, NULL, NULL);
    ...
    // 消息头
    memcpy(&head, read_buf, MESSAGE_HEAD_LEN);
    if (head.msgid != lbrss::ID_API_GetRouteResponse) {
        fprintf(stderr, "message ID error!\n");
        return lbrss::RET_SYSTEM_ERROR;
    }
    ...
    ret = rsp.ParseFromArray(read_buf + MESSAGE_HEAD_LEN, message_len - MESSAGE_HEAD_LEN);
    if (!ret) {
        fprintf(stderr, "message format error: head.msglen = %d, message_len = %d, message_len - MESSAGE_HEAD_LEN = %d, head msgid = %d, ID_GetHostResponse = %d\n", head.msglen, message_len, message_len-MESSAGE_HEAD_LEN, head.msgid, lbrss::ID_GetRouteResponse);
        return lbrss::RET_SYSTEM_ERROR;
    }
    ...
    for (int i = 0; i < rsp.host_size(); i++) {
        const lbrss::HostInfo &host = rsp.host(i);
        struct in_addr inaddr;
        inaddr.s_addr = host.ip();
...
// 系统上报host调用信息
void APIClient::report(int modid, int cmdid, const std::string &ip, int port, int retcode)
{
    // 1 封装上报消息
    ...
    // 1.1 host信息
    lbrss::HostInfo *hp = req.mutable_host();
    ...
    MsgHead head;
    head.msglen = req.ByteSizeLong();
    head.msgid = lbrss::ID_ReportRequest;
    memcpy(write_buf, &head, MESSAGE_HEAD_LEN);
    req.SerializeToArray(write_buf + MESSAGE_HEAD_LEN, head.msglen);
