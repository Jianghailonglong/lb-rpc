...
struct LBConfig
{
    ...
    // 对于每个NEW状态的modid/cmdid，多久更新一下本地路由秒
    long update_timeout;
};

// 全局配置
extern struct LBConfig lb_config;

// 与report_client通信的thread_queue消息队列
extern ThreadQueue<lbrss::ReportStatusRequest>* report_queue;

// 与dns_client通信的thread_queue消息队列
extern ThreadQueue<lbrss::GetRouteRequest>* dns_queue;

// 每个Agent UDP server的 负载均衡器路由 RouteLB
extern RouteLB * r_lb[3];
