...
// 单例对象
BufPool * BufPool::_instance = NULL;

// 用于保证创建单例的init方法只执行一次的锁
pthread_once_t BufPool::_once = PTHREAD_ONCE_INIT;


// 用户保护内存池链表修改的互斥锁
pthread_mutex_t BufPool::_mutex = PTHREAD_MUTEX_INITIALIZER;


// 构造函数 主要是预先开辟一定量的空间
// 这里BufPool是一个hash，每个key都是不同空间容量
// 对应的value是一个IOBuf集合的链表
// BufPool -->   [m4K] -- io_buf-io_buf-io_buf-io_buf...
...
BufPool::BufPool():_total_mem(0)
{
    IOBuf *prev; 
    
    // ----> 开辟4K buf 内存池
    _pool[m4K] = new IOBuf(m4K);
    if (_pool[m4K] == NULL) {
    ...
    // ----> 开辟8M buf 内存池
    _pool[m8M] = new IOBuf(m8M);
    if (_pool[m8M] == NULL) {
        fprintf(stderr, "new IOBuf m8M error");
        exit(1);
    }
