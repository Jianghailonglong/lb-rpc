...
const int ReactorBuf::length() const 
{
...
int InputBuf::read_data(int fd)
{
    int need_read;// 有多少数据可以读
    ...
    if (_buf == NULL) {
        // 如果io_buf为空,从内存池申请
        _buf = BufPool::instance()->alloc_buf(need_read);
        if (_buf == NULL) {
        ...
        if (_buf->capacity - _buf->length < (int)need_read) {
            // 不够存，从内存池申请
            IOBuf *new_buf = BufPool::instance()->alloc_buf(need_read+_buf->length);
            if (new_buf == NULL) {
...
// 取出读到的数据
const char *InputBuf::data() const 
{
...
int OutputBuf::send_data(const char *data, int datalen)
{
    if (_buf == NULL) {
        // 如果io_buf为空,从内存池申请
        _buf = BufPool::instance()->alloc_buf(datalen);
        if (_buf == NULL) {
            fprintf(stderr, "no idle buf for alloc\n");
            return -1;
        }
        ...
        if (_buf->capacity - _buf->length < datalen) {
            // 不够存，从内存池申请
            IOBuf *new_buf = BufPool::instance()->alloc_buf(datalen+_buf->length);
            if (new_buf == NULL) {
                fprintf(stderr, "no ilde buf for alloc\n");
                return -1;
            }
