...
//===================== 消息分发路由机制 ==================
typedef void msg_callback(const char *data, uint32_t len, int msgid, NetConnection *net_conn, void *user_data);
...
public:
    ...
    // 调用注册的对应的回调业务函数
    void call(int msgid, uint32_t msglen, const char *data, NetConnection *net_conn) 
    {
        // 判断msgid对应的回调是否存在
        if (_router.find(msgid) == _router.end()) {
